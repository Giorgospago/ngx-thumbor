...
export class ThumbDirective implements OnInit {
    ...
    ngOnInit(): void {
        if (this.resize) { this.ts.thumbor.resize(this.resize.width, this.resize.height); }
        if (this.autoJPG) { this.ts.thumbor.filter(`autojpg()`); }
        if (this.backgroundColor) { this.ts.thumbor.filter(`background_color(${this.backgroundColor})`); }
        if (this.blur) {
        ...
        if (this.equalize) { this.ts.thumbor.filter(`equalize()`); }
        if (this.extractFocal) { this.ts.thumbor.filter(`extract_focal()`); }
        if (this.fill) { this.ts.thumbor.filter(`fill(${this.fill})`); }
        if (this.format) { this.ts.thumbor.filter(`format(${this.format})`); }
        if (this.grayScale) { this.ts.thumbor.filter(`grayscale()`); }
        if (this.maxBytes) { this.ts.thumbor.filter(`max_bytes(${this.maxBytes})`); }
        if (this.noUpscale) { this.ts.thumbor.filter(`no_upscale()`); }
        if (this.noise) { this.ts.thumbor.filter(`noise(${this.noise})`); }
        if (this.proportion) {
        ...
        if (this.stretch) { this.ts.thumbor.filter(`stretch()`); }
        if (this.stripEXIF) { this.ts.thumbor.filter(`strip_exif()`); }
